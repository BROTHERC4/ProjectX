...
class MobileControls {
  ...
  setupControlButtons() {
    ...
    const BUTTON_Y = SCREEN_HEIGHT - 100;
    
    // Create left button
    this.leftButton = this.scene.add.image(100, BUTTON_Y, 'pointer')
    ...
    // Create right button  
    this.rightButton = this.scene.add.image(220, BUTTON_Y, 'pointer')
      .setDisplaySize(BUTTON_SIZE, BUTTON_SIZE)
      .setInteractive()
      .setAlpha(0.7)
      .setDepth(2000);
    ...
    this.fireZone.on('pointerdown', (pointer) => {
      ...
      const buttonArea = 150; // Area around buttons to ignore
      
      if (pointer.x < 50 || (pointer.x > 50 && pointer.x < 270 && pointer.y > BUTTON_Y - 50)) {
